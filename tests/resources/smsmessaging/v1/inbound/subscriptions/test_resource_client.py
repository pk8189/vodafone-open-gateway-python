...
from sms_messaging_interface.types.smsmessaging.v1.inbound.subscriptions import models
...
def test_create_201_generated_success_default():
    ...
    client = Client()
    response = client.smsmessaging.v1.inbound.subscriptions.create(
        data={
            "subscription": {
                "callback_reference": {
...
async def test_await_create_201_generated_success_default():
    ...
    client = AsyncClient()
    response = await client.smsmessaging.v1.inbound.subscriptions.create(
        data={
            "subscription": {
                "callback_reference": {
                    "callback_data": "some data useful to the requestor",
...
def test_delete_204_generated_success():
    ...
    # tests calling sync method with example data (keep comment for code generation)
    client = Client()
    response = client.smsmessaging.v1.inbound.subscriptions.delete(
...
async def test_await_delete_204_generated_success():
    ...
    # tests calling async method with example data (keep comment for code generation)
    client = AsyncClient()
    response = await client.smsmessaging.v1.inbound.subscriptions.delete(
        subscription_id="string"
    )
    adapter = TypeAdapter(None)
    adapter.validate_python(response)
